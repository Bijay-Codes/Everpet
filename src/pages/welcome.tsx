...
function CTAButtons({ navTo }: { navTo: NavigateFunction }) {
    ...
    return (
        <section className="flex flex-col gap-4">
            <span className="text-center">Our Auto login attempt [ {status === 'pending' ? 'In progress' : status === 'rejected' ? 'got Rejected' : status === 'server-issue' ? 'had some server issues please try again later' : ''} ], please login or register to continue</span>
